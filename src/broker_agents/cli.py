...
@app.command("compare-gatekeeper-stabilized-evidence")
def compare_gatekeeper_stabilized_evidence_command(
    stabilization_validation_run_id: Annotated[
        str | None,
        typer.Option(
            "--stabilization-validation-run-id",
            help="Task 128 stabilization validation run used for comparison.",
        ),
    ] = None,
    gatekeeper_re_evaluation_run_id: Annotated[
        str | None,
        typer.Option(
            "--gatekeeper-re-evaluation-run-id",
            help="Optional explicit Task 123 Gatekeeper baseline run.",
        ),
    ] = None,
    auto_latest: Annotated[
        bool,
        typer.Option(
            "--auto-latest",
            help="Use the latest stabilization validation manifest.",
        ),
    ] = False,
    outputs_root: Annotated[
        Path,
        typer.Option(
            ...
            readable=True,
            writable=True,
            help="Root containing validation, Gatekeeper, and comparison outputs.",
        ),
    ] = Path("data/outputs"),
) -> None:
    """Execute Task 129 Gatekeeper baseline vs stabilized evidence comparison."""
    if bool(stabilization_validation_run_id) == bool(auto_latest):
        raise typer.BadParameter(
            "Provide exactly one of --stabilization-validation-run-id or --auto-latest."
        )
    try:
        files = write_gatekeeper_stabilized_evidence_comparison_report(
            outputs_root=outputs_root,
            stabilization_validation_run_id=(
                None if auto_latest else stabilization_validation_run_id
            ),
            gatekeeper_re_evaluation_run_id=gatekeeper_re_evaluation_run_id,
        )
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        raise typer.BadParameter(
            f"Gatekeeper stabilized evidence comparison failed: {exc}"
        ) from exc

    report = files.report
    summary = report.comparison_summary
    table = Table(title="Gatekeeper 123 vs Stabilized Evidence Comparison")
    table.add_column("Field")
    table.add_column("Value")
    rows = (
        (
        ...
        ("Current Phase", "17 - Targeted Evidence Stabilization Layer"),
        ("Current Task", summary["current_task_name"]),
        ("Task 123 Gatekeeper Outcome", summary["task_123_gatekeeper_outcome"]),
        ("Progression Allowed", str(summary["progression_allowed"]).lower()),
        ("Persona Reviews Allowed", str(summary["persona_reviews_allowed"]).lower()),
        ("Blockers Compared", str(summary["blockers_compared"])),
        ("Blockers Improved", str(summary["blockers_improved"])),
        (
            "Blockers Partially Improved",
            str(summary["blockers_partially_improved"]),
        ),
        ("Blockers Unresolved", str(summary["blockers_unresolved"])),
        ("Recommended Next Task", report.recommended_next_task),
        ("Report Path", str(files.markdown_path)),
        ("Status", report.comparison_status),
    )
...
@app.command("close-phase-17")
def close_phase_17_command(
    gatekeeper_stabilization_re_review_run_id: Annotated[
        str | None,
        typer.Option(
            "--gatekeeper-stabilization-re-review-run-id",
            help="Task 130 re-review run used for Task 131 closure.",
        ),
    ] = None,
    auto_latest: Annotated[
        bool,
        typer.Option(
            "--auto-latest",
            help="Use the latest Gatekeeper stabilization re-review manifest.",
        ),
    ] = False,
    outputs_root: Annotated[
        Path,
        typer.Option(
            ...
            readable=True,
            writable=True,
            help="Root containing Gatekeeper stabilization re-review outputs.",
        ),
    ] = Path("data/outputs"),
) -> None:
    ...
    try:
        files = write_phase_17_closure_report(
            outputs_root=outputs_root,
            gatekeeper_stabilization_re_review_run_id=(
                None if auto_latest else gatekeeper_stabilization_re_review_run_id
            ),
        )
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        raise typer.BadParameter(f"Phase 17 closure failed: {exc}") from exc

    report = files.report
    summary = report.phase_17_closure_summary
    table = Table(title="Phase 17 Closure & Next-Step Decision")
    table.add_column("Field")
    table.add_column("Value")
    rows = (
...
@app.command("build-gatekeeper-return-input-inventory")
def build_gatekeeper_return_input_inventory_command(
    gatekeeper_return_plan_run_id: Annotated[
        str | None,
        typer.Option(
            "--gatekeeper-return-plan-run-id",
            help="Task 132 Gatekeeper return plan used for Task 133 inventory.",
        ),
    ] = None,
    auto_latest: Annotated[
        bool,
        typer.Option(
            "--auto-latest",
            help="Use the latest Gatekeeper return package plan manifest.",
        ),
    ] = False,
    outputs_root: Annotated[
        Path,
        typer.Option(
            ...
            readable=True,
            writable=True,
            help="Root containing Gatekeeper return plan and inventory outputs.",
        ),
    ] = Path("data/outputs"),
) -> None:
    """Execute Task 133 Gatekeeper return package input inventory."""
    if bool(gatekeeper_return_plan_run_id) == bool(auto_latest):
        raise typer.BadParameter(
            "Provide exactly one of --gatekeeper-return-plan-run-id or --auto-latest."
        )
    try:
    ...
    table = Table(title="Gatekeeper Return Package Input Inventory")
    table.add_column("Field")
    table.add_column("Value")
    rows = (
        (
        ...
        ("Final Progression Status", summary["final_progression_status"]),
        ("Final Persona Review Status", summary["final_persona_review_status"]),
        ("Component Inputs Total", str(summary["component_inputs_total"])),
        ("Component Inputs Ready", str(summary["component_inputs_ready"])),
        ("Evidence Artifacts Total", str(summary["evidence_artifacts_total"])),
        ("Evidence Artifacts Ready", str(summary["evidence_artifacts_ready"])),
        ("Recommended Next Task", report.recommended_next_task),
        ("Report Path", str(files.markdown_path)),
...
@app.command("build-limited-preparation-artifact-inventory")
def build_limited_preparation_artifact_inventory_command(
    limited_preparation_plan_run_id: Annotated[
        str | None,
        typer.Option(
            "--limited-preparation-plan-run-id",
            help="Task 138 Limited Preparation Governance Plan used for Task 139.",
        ),
    ] = None,
    auto_latest: Annotated[
        bool,
        typer.Option(
            "--auto-latest",
            help="Use the latest Limited Preparation Governance Plan manifest.",
        ),
    ] = False,
    outputs_root: Annotated[
        Path,
        typer.Option(
            ...
            readable=True,
            writable=True,
            help="Root containing limited preparation artifact inventory outputs.",
        ),
    ] = Path("data/outputs"),
) -> None:
    ...
    try:
        files = write_limited_preparation_artifact_inventory_report(
            ...
            limited_preparation_plan_run_id=(
                None if auto_latest else limited_preparation_plan_run_id
            ),
        )
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        raise typer.BadParameter(
    ...
    table = Table(title="Limited Preparation Artifact Inventory")
    table.add_column("Field")
    table.add_column("Value")
    rows = (
        (
        ...
        ("Limited Preparation Plan Run ID", report.limited_preparation_plan_run_id),
        ("Current Phase", "19 - Limited Preparation Governance Layer"),
        ("Current Task", summary["current_task_name"]),
        (
            "Source Gatekeeper Return Outcome",
        ...
        ("Artifact Inventory Status", report.artifact_inventory_status),
        ("Artifacts Total", str(summary["artifacts_total"])),
        (
            "Artifacts Ready For Assembly",
            str(summary["artifacts_ready_for_assembly"]),
        ),
        (
            "Artifacts Ready With Warnings",
            str(summary["artifacts_ready_with_warnings"]),
        ),
        ("Artifacts Blocked", str(summary["artifacts_blocked"])),
        ("Recommended Next Task", report.recommended_next_task),
        ("Report Path", str(files.markdown_path)),
        ("Status", report.artifact_inventory_status),
    )
    for label, value in rows:
        table.add_row(label, value)
    console.print(table)
